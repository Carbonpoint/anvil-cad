//! Render and export the logo business card sample:
//! `cargo run --release -p anvil-ui --example export_logo_card -- <out_dir>`
fn main() {
    let dir = std::path::PathBuf::from(std::env::args().nth(1).unwrap_or_else(|| ".".into()));
    std::fs::create_dir_all(&dir).unwrap();
    let doc = anvil_io::logo_card::skyline_sar_card();
    for (name, img) in
