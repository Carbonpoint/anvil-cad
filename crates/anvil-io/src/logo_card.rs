//! Logo business card sample: a name-and-logo card for a drone search and
//! rescue startup called Skyline SAR. The logo is one sketch, so it exports as a single part and can be
...
/// Placeholder names for the batch export. Pass real names on the command
/// line of the `export_logo_cards` example.
pub const SAMPLE_NAMES: [&str; 3] = ["Avery", "Jordan", "Sam"];
...
/// The card with a person's name under the company name. An empty
...
mod tests {
    ...
    fn card_builds_with_parts_and_printable_strokes() {
        ...
        let named = skyline_sar_card_for("Avery");
